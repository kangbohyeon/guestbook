...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="kangbohyun/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/kangbohyeon/guestbook.git'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/kangbohyeon/guestbook-config.git'
                }
            }
        }
            ...
            steps {
                dir('/root/sub-workspace/guestbook-config'){
                    sh '''
                        ...
                        git push "https://kangbohyeon:${GITHUB_ACCESS_TOKEN}@github.com/kangbohyeon/guestbook-config.git"
                    '''
                }
            }
        }
